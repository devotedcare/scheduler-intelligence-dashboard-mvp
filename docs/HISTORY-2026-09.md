# CLAUDE.md as it stood on 2026-09-24 — the full record

**This file is frozen and is not loaded into context.** It is the version of
[CLAUDE.md](../CLAUDE.md) from before the 2026-09-24 trim, kept verbatim.

CLAUDE.md now carries the rules, the data reference and the commands. Everything
that was cut is here: the measurements behind a constant, the incident that
produced a rule, the adversarial reviews, the approaches that were tried and
reverted, and the corrections the document made to itself.

**Read it when a rule in CLAUDE.md looks arbitrary and you need the evidence
before changing it.** Do not edit it — it is a snapshot. New decisions belong in
CLAUDE.md, and new history in CHANGELOG.md.
...
`PROMPT_VERSION` went to **2** for this: a block may hold more than one note,
every note in it must be covered, each caregiver named, and a multi-note block
may run to five sentences and 75 words. It is **4** now — see *PROMPT_VERSION 4*
below. On the live board exactly one block on 2026-09-22
...
#### PROMPT_VERSION 4 — Mitch's register, with the caregiver kept as the subject

Carlo, 2026-09-24, after comparing our output against a sample Mitch sent him:
*"I just want to almost mimic the example that Mitch gave to me but Edna
(caregiver) blah blah is better than Caregiver blah blah."*

The sample is worth keeping in mind because it is the target shape. Thirteen
clients, one paragraph each, **413 words for a whole day** against our 1,039:

```
Brown, Mary Lou
Client was assisted with morning routine including bed-making, medication
reminders, and review of daily schedule. Transported to an outside facility
and returned home.
```

**It is not the same data**, and that was checked before any of it was acted
on: four of its clients have no notes in our mirror at all, Freese's newest is
2026-09-18, and it omits eight clients we summarised on 2026-09-23. Margrith
Hawkins reads *"No care notes available"* there against a 62-word note here.
So nobody rendered the same note better — but both files are one house style
over a full day, and the STYLE comparison stands.

**What we took: the register.** Flowing prose that groups related care rather
than a timestamped log.

**What we refused: the anonymity.** The sample carries **zero caregiver names
in 413 words**. Ours opens **21 of 21** blocks with one. *"Transported to an
outside facility"* cannot be rung up; *"Margarita clocked in at 8pm. Yvette
reported…"* can. Every follow-up the sample generates starts with a lookup we
had already done.

We also kept the **AM/PM split** (the sample merges the day), and the
**all-clear register** — *"Routine day."* The sample has no way to say a shift
was fine, so an ordinary day reads at the same weight as an agitated one.

##### What each version actually changed

| | v2 | v3 | v4 |
|---|---|---|---|
| opens with a name | 19/20 | 21/21 | **21/21** |
| passive voice | 4/20 | 2/21 | **2/21** |
| leaks our block structure | — | **1** | **0** |
| lists 3+ readings of one measurement | — | **2** | **1** |
| one-note blocks over the 45-word cap | 12/19 | 14/20 | **13/20** |

**v3 fixed the register and lost ground on length.** The cap was buried under a
long register block and a seven-line figure rule, and *"flowing prose that
groups related care"* pulls towards longer sentences. It also produced two
regressions worth naming because both are the shape of thing a prompt edit
causes:

- **it enumerated eight blood-pressure readings** where v2 had written
  *"ranging 155–187 systolic"*. The figure rule said which figures matter but
  nothing about the same measurement taken nine times.
- **it told the desk about our plumbing** — *"Reyna's note from a later shift
  (mislabeled in AM block)"*. The model had correctly noticed that an AM block
  held a note describing a 6pm start, and said so on a clinical page.

v4 closed both. Ziad's PM block now reads *"blood pressure … ran elevated
155–187 systolic throughout the shift … called Leila at 8 PM to report;
Leila informed Suhad and asked for hourly checks"* — the range plus what was
done about it, which is the whole point.

##### The four fixes, and why each was a defect rather than taste

1. **The PM window contradicted itself.** `shiftOf()` files 14:00–06:00 as PM,
   but the prompt's opening line said *"PM is 2pm-10pm"* while a paragraph four
   lines later said *"The PM window runs to 6am"*, and `blockFor()` stamped
   every block *"PM (2:00 PM - 10:00 PM)"*. The model was being told a window
   that excludes the hours most PM blocks are about — Jose's ran to 7:00 AM,
   Ziad's ended 6:20 AM, Virginia's *"Endorsed at 6am still in kitchen"*.
   **Introduced 2026-09-23 with the multi-note rule** and caught only by
   holding our output beside somebody else's.
2. **The figure rule had no test.** *"Keep the times, dates and figures that
   matter"* is unfalsifiable, so anything numeric was copied. Measured on
   2026-09-23: one-note blocks WITH a figure averaged 54 words and 11 of 15
   broke the cap; without, 36 words and 1 of 4. The rule is now operational —
   *a figure earns its place only if a different value would change what the
   office does today* — with the repeated-measurement clause beside it.
3. **The caregiver's name came out of the note body.** One caregiver was
   *"Aliyah"* in the AM block and *"Aaliyah"* in the PM block of the same
   household. `blockFor()` now passes the **first name from
   `care_notes.caregiver_name`** — the agency's own spelling — and the prompt
   prefers it over anything typed inside a note at 3am. It also ended
   *"Donmar Erick Villanueva's shift with Jose Ortiz from 6:55 PM"*, fourteen
   words of identification against a 45-word cap; it is *"Donmar"* now.
4. **It characterised the caregiver.** *"Edna reports very minimal detail"* is
   a performance judgement, model-written, saved in a clinical table, and the
   one line on the page a family or a licensing reader would read as an
   accusation. The note may be thin — say that about the NOTE. Same block now:
   *"Edna watered indoor and outdoor plants, fixed the bed, prepared coffee for
   the next morning, and had lunch and dinner ready. Attempted a bike ride but
   returned due to heat."*

##### SUPERSEDED at v6 — the 45-word-per-BLOCK cap is gone

The cap below was the wrong UNIT and was replaced on 2026-09-24; see
*PROMPT_VERSION 6* at the end of this section. Kept because the measurement
still stands: Haiku does not count words to order.

##### KNOWN: the 45-word cap was aspirational on Haiku

**41 of 65 saved blocks are over it; the median is 50 words.** Two prompt
passes moved it from 14/20 to 13/20 — that is noise, not a fix. Raising the
budget's prominence and giving it a worked example did not make Haiku count.

It is left as it stands because the output reads well at ~50 words and the
`MAX_CHARS` 700 guard is what actually protects the page. Three honest options
if it ever matters: accept it, restate the cap at 60 so the prompt stops
claiming something untrue, or move `CARENOTES_MODEL` to Sonnet — better
instruction-following at roughly 3× the cost, about 5¢ a date against 1.8¢.

One block still lists three readings (2026-09-16 PM) and Margrith Hawkins still
carries a breakfast inventory. Both are the same unfixed edge: the model keeps
detail the note dwells on.

##### Re-running the saved dates

`PROMPT_VERSION` rewrites a row on its next open, so nothing had to be done.
All three dates were regenerated by hand anyway, at Carlo's request, so the
desk sees the new wording immediately rather than on first open:

```
POST /functions/v1/carenotes-summary  {"day":"YYYY-MM-DD"}
2026-09-16  22 regenerated      2026-09-22  22      2026-09-23  21
```

65 blocks, three model calls, about 5 cents.

##### PROMPT_VERSION 6 — a GENERAL summary, and no numbers at all

Carlo, 2026-09-24: *"still too wordy for the schedulers… Mitch just wants a
summary of what generally happened based on the Care Notes rather than a very
detailed summary, since if she wants she can just open the history."*

Verified before acting, on the sample's own unit — one client's whole day:

| | clients | total | median | mean |
|---|---|---|---|---|
| Mitch's sample | 11 | 399w | 40 | **36.3** |
| ours at v4 | 15 | 1,167w | 68 | **77.8** |
| **ours at v6** | 16 | **851w** | **59** | **53.2** |

| per 100 words | sample | v4 | **v6** |
|---|---|---|---|
| clock times | 0.0 | 3.8 | **0.1** |
| numbers | 0.0 | 8.7 | **0.0** |

His reason held too: the raw notes behind *View Original Notes* are 4,013 words
for that day, and v4 was reprinting **29%** of them.

###### The cause was the UNIT, not the model

The cap was **45 words per BLOCK** where the sample is **~36 per CLIENT-DAY**,
so a client with an AM and a PM block was licensed 90 against the sample's 36.

###### WRITE NO NUMBERS — a safety rule as much as a length one

No clock times, vitals, blood sugars, intake or output volumes, doses, or counts
of brief changes and bathroom trips. Describe the **pattern**: *"restless and up
to the toilet all night"*, *"ate almost nothing at lunch"*.

**One exception: a figure the note shows somebody REACTING to** — a dose
repeated because the first did nothing, a reading a nurse or the family was rung
about. Once, with what was done about it.

The second reason for the rule is the one worth keeping. An audit of all 67
saved blocks found exactly **one invented reading** — a summary opened *"BP
128/81"* on a note recording only 142/79. One in 67 is small, but **a figure we
do not print is a figure we cannot get wrong**, and a scheduling desk cannot act
on a reading anyway. Since v6 the audit reads **0**.

###### The budget is split in CODE, because the model cannot divide

`DAY_BUDGET_WORDS = 40` is the client's whole day. v5 said so in the prompt, in
those words, and it did not work:

```
              v5 result           told
ONE block     median 41 words     ~40      on target
TWO blocks    median 83 words     ~20 each each block wrote its own 40
```

It cannot hold a budget across blocks it writes separately. So `askClaude()`
counts each client's blocks, divides, and `blockFor()` stamps a **`Budget: N
words`** line into the header. Deterministic beats instructed.

###### KNOWN: Haiku overshoots a stated budget by about three quarters

At v6, one-block clients land on **median 37 against a stated 40** — right. Two-
block clients are told 20 each and write about 35, landing at **median 71**.
That is the whole of the remaining gap to the sample.

Three passes moved the mean 77.8 → 59.9 → 53.2 and the next one will not do
much. Left as it is, with two levers if it ever matters:

- **lower `DAY_BUDGET_WORDS`** to compensate for the overshoot — tuning a
  constant against one model's behaviour, so it needs re-checking on a switch
- **`CARENOTES_MODEL=claude-sonnet-5`** — better instruction-following, ~5¢ a
  date against 1.8¢, one secret, no redeploy

###### The worked examples are INVENTED people now

v4's examples used **real names and real readings** off the live board — *Edna*,
*Mary Lou*, *"Krissa called Leila"*, *155–187*. An illustration a model can
mistake for data is a hazard, and the prompt now says so in terms: *never reuse
a name, a reading or a time that appears in these instructions.* The examples
are Rosa and Mr Alder, who do not exist.

###### A near-miss worth recording: the audit script had the UTC bug

The first fabrication audit reported wholesale invention — Brenda's *117/70*,
Ziad's *Krissa* and *155–187*, Diann's *900cc*. **All of it was real and
correctly dated.** The audit bucketed notes by slicing the **UTC** timestamp,
which files a 19:00 Pacific note under the *next* day; Krissa Mae Pacada's note
of 2026-09-16 19:00 PT contains every reading the report called invented.

This is the trap CLAUDE.md already records against the care-notes sync, the
caregiver calendar and the open-shift mirror — made this time in the checking
tool, which is the one place nobody thinks to look. **Any script that buckets
`care_notes` by day must use `toLocaleDateString('en-CA', {timeZone:
'America/Los_Angeles'})`, exactly as `dayKey()` does.** Re-run correctly, 67
blocks gave one bad figure, not a systemic failure — and a false alarm about
fabricated clinical data would have been far more expensive than the wordiness
it was raised alongside.
