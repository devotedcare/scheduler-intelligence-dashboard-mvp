...
- **City strings are dirty.** `CAMARILLO`, `Camarilllo`, `"Oxnard "`, `oxnard`
  ...
  *Superseded 2026-09-21: Find Coverage shows drive time from `DRIVE_TIMES`
  (regenerated by `scripts/drive-times.js`), not from `CITY`, and it already
  knows every home city on the roster except "Los Angeles". Add a new city
  there.*
